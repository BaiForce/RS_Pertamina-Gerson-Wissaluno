...
@endphp
<div class="card m-3 border-0 shadow" style="width: 90vw; max-width: 400px; border-radius: 10px;">
        ...
        <div class="col-8">
            <div class="card-body">
                ...
                <p class="card-text text-start">
                    <!-- @if ($item->status == 1)
                    <span class="badge bg-warning text-start">Sedang Dalam Peminjaman</span>
                    @else
                    <span class="badge bg-info text-start">Sudah Dikembalikan</span>

                    @endif -->
                    <span class="badge bg-info text-start">Peminjaman {{ $date }}</span><br>
                    <span class="badge bg-info text-start">Pengembalian {{ $newTime }}</span>
                </p>
